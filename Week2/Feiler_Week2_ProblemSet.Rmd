...
```{r confirm bootstrap, echo = FALSE}
...
bootTmeans_sd <- sd(rowMeans(bootT)-rowMeans(bootC))

treat_con_var <- sqrt(var(treatment)/length(treatment)+var(control)/length(control))

```
  
For this bootstrap run, the difference  between the the standard deviation of $\overline{Treatment} \space - \space \overline{Control}$ and $\sqrt{\frac{Var[X_{T}]}{n_{T}}+\frac{Var[X_{C}]}{n_{C}}}$ is `r abs(bootTmeans_sd - treat_con_var)`, which is acceptable.
...
```{r 2a answer, echo = FALSE}
# Find the standard error of the treatment bootstrap
sterr_treat <- sd(rowMeans(bootT))
...
```{r 2b answer, echo = FALSE}
# Find the standard error of the control bootstrap
sterr_con <- sd(rowMeans(bootC))
...
upper2d <- mean(bootDiff)+(1.96*sterr_diff)
lower2d <- mean(bootDiff)-(1.96*sterr_diff)

```

The 95^th confidence interval for the difference in means is (`r paste(round(lower2d, digits = 2), ", ", round(upper2d, digits = 2), sep = "")`).  
...
The 95<sup>th</sup> confidence interval for the difference in means is (`r paste(round(lower2e, digits = 2), ", ", round(upper2e, digits = 2), sep = "")`).  
