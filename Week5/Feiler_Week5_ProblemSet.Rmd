...
>$H_{null}: \overline{x}_{metal} = \overline{x}_{electronic}$ ; $H_{alt}: \overline{x}_{metal} \neq \overline{x}_{electronic}$
$$
\begin{split}
...
metal <- list(12.70, 3.21, 344)
names(metal) <- c("mean", "sd", "n")

# Penguins with electronic tags
electronic <- list(11.60, 4.53, 512)
names(electronic) <- c("mean", "sd", "n")

# Create list for summary statistics
penguinHypTest <- vector("list", length = 4)
names(penguinHypTest) <- c("stErr", "t", "dof", "p")

# Calculate standard error of the means
penguinHypTest$stErr <- sqrt(((metal$sd^2)/metal$n) + ((electronic$sd^2)/electronic$n))

# Calculate the test statistic
penguinHypTest$t <- (metal$mean - electronic$mean)/penguinHypTest$stErr

# Calculate the degrees of freedom
top <- ((metal$sd^2/metal$n)+(electronic$sd^2/electronic$n))^2
bottom1 <- ((metal$sd^2/metal$n)^2)/(metal$n-1)
bottom2 <- ((electronic$sd^2/electronic$n)^2)/(electronic$n-1)
